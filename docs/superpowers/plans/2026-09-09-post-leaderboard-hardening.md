...
- **Phase 7** (Switch Track fire-state) — ✅ done, merged + pushed (`findmysensi` `main`, merge `2d672c9`). All automated gates green (format, lint, typecheck, boundaries, decisions, protocol-freeze, 691 unit tests, build, 11 E2E); `findmysensi-secure` needed no change. Detailed decisions and acceptance evidence: `docs/superpowers/plans/2026-09-09-switch-track-fire-state.md`. `scoringVersion` bumped to `1` for a fresh leaderboard partition (`switch-track:scenario-0:scoring-1:...`); `engineVersion` and `analyticsVersion` stay `1`. **Real-mouse verification still owed** — hold LMB over a target accrues damage, release stops it, a grid tap still scores.
- **Phase 8** — owner-run manual benchmark.
- **Phase 9** (M17 release) — unblocked. Remaining: real-mouse checks for Phase 5 (heatmap) and Phase 7 (held fire), Phase 8 benchmark, then tag both repos `v1.0.0`.
